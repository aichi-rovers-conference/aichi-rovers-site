// src/app/login/page.tsx — Server Component（★APIのPOSTはここに書かない）
...
export default async function ExecLoginPage({
  ...
  else if (errParam && errParam !== "missing" && errParam !== "invalid")
    errorText = "ログインに失敗しました。";
  if (!errorText && authParam === "expired")
    errorText = "ログインの有効期限が切れました。もう一度サインインしてください。";
  if (!errorText && authParam === "required")
    errorText = "サインインが必要です。アカウントでログインしてください。";
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      ...
      <section className="mx-auto max-w-md px-4 py-10">
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {/* 成功時は /api/auth/login が Set-Cookie → 200 HTML でJS遷移 */}
          <form
