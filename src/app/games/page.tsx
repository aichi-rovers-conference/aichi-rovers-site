...
export default function GamesPage() {
  ...
  return (
    <div className="w-full bg-white min-h-screen">
      {/* ===== PC: 既存デザインそのまま（md以上で表示） ===== */}
      <header className="hidden md:flex select-none w-full items-center justify-between p-4 bg-white shadow z-50 relative">
      ...
      {/* ===== Mobile: デザイン変更（タイトルを“必ず1行”で省略表示） ===== */}
      ...
      <div className="mx-auto max-w-6xl px-6 md:px-16 py-6 flex items-center gap-3 text-slate-700">
        <Link
          href="/"
          className="group inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white/80 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-50"
        >
...
function GamesHero({ hideText }: { hideText: boolean }) {
  ...
  return (
    <div ref={ref} className="select-none relative w-full h-[36vh] md:h-[52vh] overflow-hidden">
      ...
      >
        <h1 className="text-white text-4xl md:text-7xl font-extrabold mb-2 drop-shadow-lg tracking-tight">
          ミニゲーム
        </h1>
        <p className="text-white/90 text-base md:text-2xl font-semibold">気分転換に、さくっとプレイ</p>
      </motion.div>
    </div>
  );
}
