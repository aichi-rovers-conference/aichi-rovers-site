      ...
      <header className="select-none w-full flex items-center justify-between p-4 bg-white shadow z-50 relative">
        <div className="flex items-center">
          ...
          >
            <AnimatePresence mode="wait" initial={false}>
              {isOpen ? (
                <motion.span key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ type: "spring", stiffness: 400, damping: 28 }}>
                  <X size={28} className="text-black" />
                </motion.span>
              ) : (
                <motion.span key="menu" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ type: "spring", stiffness: 400, damping: 28 }}>
        ...
        <nav className="hidden md:flex space-x-6 text-gray-600">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.path} href={item.path} className={`px-4 py-2 rounded-lg transition ${isActive ? "text-black font-bold" : "text-gray-500 hover:text-black"}`}>{item.name}</Link>
            );
          })}
        </nav>
        ...
        <div className={`absolute top-16 left-0 w-full bg-white shadow-md flex flex-col space-y-2 p-4 md:hidden transform transition-all duration-300 ease-in-out z-50 ${isOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}>
          {NAV_ITEMS.map((item) => {
            const isActive = pathname === item.path;
            return (
              <Link key={item.path} href={item.path} className={`px-4 py-2 rounded-lg transition ${isActive ? "text-black font-bold" : "text-gray-500 hover:text-black"}`} onClick={() => setIsOpen(false)}>{item.name}</Link>
            );
          })}
        </div>
      </header>
